...
4. **Tokens enumerable.** Every token the component consumes appears in `meta.tokens`. The default convention is component-scoped tokens (`button-primary-bg`, `button-primary-bg-hover`) — states encoded in the name. If your project layers on a semantic palette (Tailwind v4, shadcn), list the semantic utilities the component consumes (`bg-primary`) and let interaction modifiers (`hover:bg-primary/90`) carry state. See [Token architecture variants](../agentic-design-systems/SKILL.md#token-architecture-variants).
