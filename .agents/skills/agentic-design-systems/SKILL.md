...
For per-component `.meta.ts` scaffolding specifically, switch to the [`ai-component-metadata`](../ai-component-metadata/SKILL.md) skill — it produces `.meta.ts` files matching this schema, with a Python scaffolder that auto-fills `component`, `props`, and best-effort `variants.axes`.
